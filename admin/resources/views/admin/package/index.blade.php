...
<package-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/packages') }}'" inline-template>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/packages/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                        {{ trans('admin.package.actions.create') }}</a>
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    {{-- <th is='sortable' :column="'description'">{{ trans('admin.package.columns.description') }}
                                    </th> --}}
                                    <th is='sortable' :column="'status'">{{ trans('admin.package.columns.status') }}
                                    </th>
                                    <th is='sortable' :column="'booking_gap'">
                                        {{ trans('admin.package.columns.booking_gap') }}</th>
                                    <th is='sortable' :column="'start_date'">
                                        {{ trans('admin.package.columns.start_date') }}</th>
                                    <th is='sortable' :column="'end_date'">{{ trans('admin.package.columns.end_date') }}
                                    </th>
                                    ...
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="9">
                                        <span
                                            ...
                                            @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                @click="onBulkItemsClickedAll('/admin/packages')"
                                        ...
                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                @click="bulkDelete('/admin/packages/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                                        ...
                                        <div class="row no-gutters">
                                            <div class="col-auto">
                                                <a class="btn btn-sm btn-spinner btn-info"
                                                    :href="item.view_package_booking"> <span
                                                        class="text-success">@{{item.package_bookings}}</span> <i
                                                        class="nav-icon icon-magnet"></i></a>
                                            </div>
                                            <div class="col-auto">
                                                <a class="btn btn-sm btn-spinner btn-info"
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ url('admin/packages/create') }}"
