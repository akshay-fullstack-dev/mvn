...
@section('title', trans('admin.referral-amount.actions.index'))

@section('body')

<referral-amount-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/referral-amounts') }}'" inline-template>

    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-align-justify"></i> {{ trans('admin.referral-amount.actions.index') }}
                    @if (!$data->count()>0)
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/referral-amounts/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                        {{ trans('admin.referral-amount.actions.create') }}</a>
                </div>
                @endif
                ...
                <table class="table table-hover table-listing">
                    <thead>
                        <tr>
                            ...
                            <th is='sortable' :column="'id'">{{ trans('admin.referral-amount.columns.id') }}
                            </th>
                            <th is='sortable' :column="'referral_amount'">
                                {{ trans('admin.referral-amount.columns.referral_amount') }}</th>
                            ...
                            <td class="bg-bulk-info d-table-cell text-center" colspan="4">
                                <span
                                    ...
                                    @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                        @click="onBulkItemsClickedAll('/admin/referral-amounts')"
                                ...
                                {{-- <span class="pull-right pr-2">
                                    <button class="btn btn-sm btn-danger pr-3 pl-3"
                                        @click="bulkDelete('/admin/referral-amounts/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                </span> --}}
                ...
                <div class="no-items-found" v-if="!collection.length > 0">
                    ...
                    <a class="btn btn-primary btn-spinner" href="{{ url('admin/referral-amounts/create') }}"
