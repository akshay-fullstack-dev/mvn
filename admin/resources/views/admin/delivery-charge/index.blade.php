...
@section('title', trans('admin.delivery-charge.actions.index'))

@section('body')
<delivery-charge-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/delivery-charges') }}'" inline-template>

    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-align-justify"></i> {{ trans('admin.delivery-charge.actions.index') }}
                    @if (!$data->count() > 0)
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/delivery-charges/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                        {{ trans('admin.delivery-charge.actions.create') }}</a>
                    @endif
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' :column="'id'">{{ trans('admin.delivery-charge.columns.id') }}
                                    </th>
                                    <th is='sortable' :column="'customer_delivery_charge'">
                                        {{ trans('admin.delivery-charge.columns.customer_delivery_charge') }}</th>
                                    <th is='sortable' :column="'vendor_delivery_charge'">
                                        {{ trans('admin.delivery-charge.columns.vendor_delivery_charge') }}</th>
                                    ...
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                        <span
                                            ...
                                            @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                @click="onBulkItemsClickedAll('/admin/delivery-charges')"
                                        ...
                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                @click="bulkDelete('/admin/delivery-charges/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in collection" :key="item.id"
                                    ...
                                    <td>@{{ item.id }}</td>
                                    <td>@{{ item.customer_delivery_charge }}</td>
                                    <td>@{{ item.vendor_delivery_charge }}</td>
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ url('admin/delivery-charges/create') }}"
